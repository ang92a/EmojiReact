...
// - компонент Button плохое название - button это кнопка, а твой компонент отрисовывает целую пагинацию
// - убрать console.log из кода
// - вынести select в отдельный компонент
// - все хуки должны быть вверху компонента
// - функция по удалению дубликатов должна вызываться 1 раз, а не при каждой перерисовке компонента
// - const numberHandler = (num) => setCurrentNumber(num); необходимость данной функции?
// - в компонент Main нужно передавать готовый для отрисовки массив а не обрезать его внутри
// - let Arrey плохое название переменной